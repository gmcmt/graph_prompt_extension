...
def train(pretrain_model, model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        embedding=pretrain_model(graph,graph_len)
        ...
        pred = model(embedding, graph_len)
    ...
    if logger:
        logger.info("epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\treg loss: {:0>10.6f}\tbp loss: {:0>16.6f}\tacc: {:0>16.6f}".format(
            epoch, config["epochs"], data_type, mean_reg_loss, mean_bp_loss,mean_acc))

    gc.collect()
    return mean_reg_loss, mean_bp_loss, total_time, acc


def evaluate(pretrain_model, model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    epoch_step = len(data_loader)
    total_reg_loss = 0
    total_step = config["epochs"] * epoch_step
    total_bp_loss = 0
    ...
    with torch.no_grad():
        ...
        if writer:
            ...
            writer.add_scalar("%s/BP-%s-epoch" % (data_type, config["bp_loss"]), mean_bp_loss, epoch)
        if logger:
            logger.info("epoch: {:0>3d}/{:0>3d}\tdata_type: {:<5s}\treg loss: {:0>10.6f}\tbp loss: {:0>16.6f}\tacc: {:0>10.6f}".format(
                epoch, config["epochs"], data_type, mean_reg_loss, mean_bp_loss,mean_acc))
    ...
    gc.collect()
    return mean_reg_loss, mean_bp_loss, evaluate_results, total_time, accuracy,macrof,weightedf
...
if __name__ == "__main__":
    ...
    for data in tenfold:
        ...
        model = graph_finetuning_layer(train_config["graph_finetuning_input_dim"],
                                       train_config["graph_finetuning_output_dim"])
        ...
        for epoch in range(train_config["epochs"]):
            for data_type, data_loader in data_loaders.items():
                ...
                if data_type == "train":
                    mean_reg_loss, mean_bp_loss, _time, accfold = train(pre_train_model, model, optimizer, scheduler, data_type, data_loader, device,
