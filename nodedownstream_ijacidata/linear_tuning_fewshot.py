...
def pre_train(model, data_loader, device, config):
...
def train(model, optimizer, scheduler, data_type, device, config, epoch, label_num, pretrain_embedding, node_label, logger=None, writer=None):
    ...
    pred=model(pretrain_embedding,0)
...
def evaluate(model, data_type, device, config, epoch, c_embedding, label_num, pretrain_embedding, node_label, debug=False,logger=None, writer=None):
    ...
    pred = model(pretrain_embedding, 0)*train_config["scalar"]
...
if __name__ == "__main__":
    ...
    graph=dgl.load_graphs(train_config["graph_dir"])[0][0]
    ...
    for count in range(train_config["few_shot_tasknum"]):
        pre_data_loaders = OrderedDict({"train": None, "dev": None})
        if all([os.path.exists(os.path.join(train_config["save_data_dir"],
                                            "%s_%s_dataset.pt" % (
                                                    data_type,
                                                    "dgl" if train_config["pretrain_model"] in ["RGCN", "RGIN",
                                                                                                "GAT", "GCN",
                                                                                                "GraphSage",
                                                                                                "GIN"] else "edgeseq")))
                for data_type in pre_data_loaders]):

            logger.info("loading data from pt...")
            for data_type in pre_data_loaders:
                if train_config["pretrain_model"] in ["RGCN", "RGIN", "GAT", "GCN", "GraphSage", "GIN"]:
                    ...
                    pre_data_loader = DataLoader(dataset,
                    ...
                    pre_data_loader = DataLoader(dataset,
                                             ...
                                             pin_memory=data_type == "train")
                pre_data_loaders[data_type] = pre_data_loader
                logger.info("data (data_type: {:<5s}, len: {}) generated".format(data_type, len(dataset.data)))
                logger.info(
                    "data_loader (data_type: {:<5s}, len: {}, batch_size: {}) generated".format(data_type,
                                                                                                len(pre_data_loader),
                                                                                                train_config[
                                                                                                    "batch_size"]))
        else:
            data = pretrain_load_data(train_config["graph_dir"], train_config["graphslabel_dir"],
            ...
            for data_type, x in data.items():
                if train_config["pretrain_model"] in ["RGCN", "RGIN", "GAT", "GCN", "GraphSage", "GIN"]:
                    ...
                    if os.path.exists(
                            os.path.join(train_config["save_data_dir"], "%s_edgeseq_dataset.pt" % (data_type))):
                        dataset = EdgeSeqDataset(list())
                        dataset.load(os.path.join(train_config["save_data_dir"], "%s_edgeseq_dataset.pt" % (data_type)))
                    else:
        ...
        c_embedding = None
        data_loaders=pre_data_loaders
        for epoch in range(train_config["epochs"]):
            if epoch==0 or (epoch!=0 and train_config["pretrain_model"]):
                for data_type, pre_data_loader in pre_data_loaders.items():
                    if data_type == "train":
                        pretrain_embedding = pre_train(pre_train_model, pre_data_loader, device, train_config)
            ...
            for data_type, data_loader in data_loaders.items():
                ...
                if data_type == "train":
                    mean_reg_loss, mean_bp_loss, _time, accfold,c_embedding = train(model, optimizer, scheduler, data_type, device,
                    ...
                    mean_reg_loss, mean_bp_loss, evaluate_results, _time,accfold, macroFfold, weightedFfold,c_embedding = \
                        evaluate(model, data_type, device, train_config, epoch, c_embedding,
        ...
        for data_type, data_loader in data_loaders.items():
            mean_reg_loss, mean_bp_loss, evaluate_results, _time, acctest, macroFtest, weightedFtest,c_embedding = \
                evaluate(model, data_type, device, train_config, epoch, best_c_embedding,
