...
def train(pretrain_model, model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    gc.collect()
    return mean_reg_loss, mean_bp_loss, total_time, mean_acc
...
def evaluate(pretrain_model, model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    gc.collect()
    return mean_reg_loss, mean_bp_loss, evaluate_results, total_time, mean_acc,mean_macrof,mean_weighted
...
if __name__ == "__main__":
    ...
    for count in range(train_config["few_shot_tasknum"]):
        ...
        model = graph_finetuning_layer(train_config["gcn_hidden_dim"]*train_config["gcn_graph_num_layers"],
                                       train_config["graph_label_num"])
        ...
        for data_type, data_loader in data_loaders.items():
            mean_reg_loss, mean_bp_loss, evaluate_results, _time, acctest, macroFtest, weightedFtest = evaluate(
