...
class model_weight(nn.Module):
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.uniform_(self.temp, a=0.0, b=0.1)
    def forward(self, graph_adj,weight_id):
        ...
        if weight_id == 0:
            graph_adj = (graph_adj*(temp[0]+0.25))
        elif weight_id == 1:
            graph_adj = (graph_adj*(temp[1]+0.25))
        elif weight_id == 2:
            graph_adj = (graph_adj*(temp[2]+0.25))
        else:
            graph_adj = (graph_adj*(temp[3]+0.25))
