...
from gin_downstream import GIN_P
from gin_local import GIN
from gcl import GCL
from gcn import GCN
from gat import GAT
from graphsage import Graphsage
...
def pre_train(model_0 ,graph, device, config,self_weight = None):
    epoch_step = 1
    ...
    x_1,pred_1 = model_0(graph,graph_len,0,train_config["scalar"])
    x_2,pred_2 = model_0(graph,graph_len,1,train_config["scalar"])
    x_3,pred_3 = model_0(graph,graph_len,2,train_config["scalar"])
    x_4,pred_4 = model_0(graph,graph_len,3,train_config["scalar"])
    # pred_1=F.sigmoid(pred_1)
    # pred_2=F.sigmoid(pred_2)
    # pred_3=F.sigmoid(pred_3)
    # pred_4=F.sigmoid(pred_4)
    
  
    

    # graph_output = model_0(graph.ndata["feature"],graph.number_of_nodes())      
    # graph.ndata["feature"]=graph_output
    ...
    if config["nhop_neighbour"]==0:
        ...
        for i in range(config["nhop_neighbour"]):
            pred_1 = torch.matmul(adj, pred_1)
            pred_2 = torch.matmul(adj, pred_2)
            pred_3 = torch.matmul(adj, pred_3)
            pred_4 = torch.matmul(adj, pred_4)
    return pred_1,pred_2,pred_3,pred_4
from fvcore.nn import FlopCountAnalysis, parameter_count_table


#pretrain_embedding是该部分需要的embedding；node_label也是该部分节点对应的nodelabel
def train(weight_, optimizer, optimizer_2, scheduler, data_type, device, config, epoch, label_num, pretrain_embedding_1, pretrain_embedding_2, pretrain_embedding_3,pretrain_embedding_4,node_label, logger=None, writer=None,mix_="weighted_sum",mix_num=0.1,man_weight=None):
    epoch_step = 1
    total_step = config["epochs"] * epoch_step
    total_reg_loss = 0
    total_bp_loss = 0
    ...
    #结合两个embedding
    # if mix_=="weighted_sum":
    #     #两个embedding权重和为1
    #     embedding = mix_num*embedding + (1-mix_num)*pretrain_embedding_1
    # elif mix_ =="decay":
    #     embedding = embedding + pretrain_embedding_1*(mix_num)
    # else:
    #     embedding = embedding + pretrain_embedding_1

    embedding = weight_(pretrain_embedding_1,0) + weight_(pretrain_embedding_2,1) + weight_(pretrain_embedding_3,2) + weight_(pretrain_embedding_4,3) 
    ...
    bp_loss = bp_loss.detach_().requires_grad_(True)
    ...
    if (config["update_every"] < 2 or 0 % config["update_every"] == 0 or 0 == epoch_step - 1):
        if config["max_grad_norm"] > 0:
            torch.nn.utils.clip_grad_norm_(model_0.parameters(), config["max_grad_norm"])
          
            torch.nn.utils.clip_grad_norm_(weight_.parameters(), 5)
...
def evaluate(weight_, data_type, device, config, epoch, c_embedding, label_num, pretrain_embedding_1,pretrain_embedding_2,pretrain_embedding_3,pretrain_embedding_4, node_label, debug=False,logger=None, writer=None,mix_="weighted_sum",mix_num=0.1,man_weight=None):
    epoch_step = 1
    total_reg_loss = 0
    total_step = config["epochs"] * epoch_step
    total_bp_loss = 0
...
if __name__ == "__main__":
    ...
    # adj_self_weight = [1,1.1,1.3,1.5,1.7,1.9,2]
    adj_self_weight = [1]
 
 

    record = {1:"1",0.1:"01",0.3:"03",0.5:"05",0.7:"07",0.9:"09",0:"0",1.1:"11",1.3:"13",1.5:"15",1.7:"17",1.9:"19",2:"2",0.2:"02",0.4:"04",0.6:"06",0.8:"08",10:"10",0.01:"001",0.001:"0001",0.0001:"00001"}
    for i in range(1, len(sys.argv), 2):
        ...
        try:
            ...
            pass
        man_weight = []
        man_weight.append(train_config["p1"])
        man_weight.append(train_config["p2"])

        man_weight.append(train_config["p3"])

        man_weight.append(train_config["p4"])

    for weight in range(len(adj_self_weight)):
        mix_ = train_config["mix_"]
        mix_num = train_config["mix_num"]
        ...
        for times in range(5):
            ...
            if train_config["pretrain_model"] == "GIN":
                pre_train_model_d = GCL(train_config["node_feature_dim"],train_config["gcn_hidden_dim"],train_config)
                pre_train_model_d.load_state_dict(torch.load(os.path.join(save_pretrain_model_dir, "%s_flickr_GCL_%s.pt"%(train_config["d_p"],times))))
                torch.save(pre_train_model_d.gin.state_dict(), os.path.join(save_pretrain_model_dir, "gin_GCL_%s.pt"%(times)))
            ...
            for count in range(train_config["few_shot_tasknum"]):
                ...
                if train_config["prompt"] == "FEATURE-WEIGHTED-SUM":
                    model_0 = GIN_P(train_config)
                   
                    weight_ = model_weight()
                    # weight_1 = weight_w()
                    # weight_2 = weight_w()
                    # weight_3 = weight_w()
                    # weight_4 = weight_w()
                ...
                model_0.load_state_dict(torch.load(os.path.join(save_pretrain_model_dir, 'gin_GCL_%s.pt'%(times)),map_location='cuda:0'),strict = False)
               
                weight_ = weight_.to(device)
                # prompt_weight.append(weight_1)
                # prompt_weight.append(weight_2)
                # prompt_weight.append(weight_3)
                # prompt_weight.append(weight_4)
                pretrain_embedding_1, pretrain_embedding_2, pretrain_embedding_3, pretrain_embedding_4 = pre_train(model_0,graph, device, train_config,self_weight=adj_self_weight[weight])
                ...
                pretrain_embedding_1 = pretrain_embedding_1.to(device)
                pretrain_embedding_2 = pretrain_embedding_2.to(device)
                pretrain_embedding_3 = pretrain_embedding_1.to(device)
                pretrain_embedding_4 = pretrain_embedding_1.to(device)
                ...
                trainemb_1 = mask_select_emb(pretrain_embedding_1, trainmask, device)
                valemb_1 = mask_select_emb(pretrain_embedding_1, valmask, device)
                testemb_1 = mask_select_emb(pretrain_embedding_1, testmask, device)
                
                trainemb_2 = mask_select_emb(pretrain_embedding_2, trainmask, device)
                valemb_2 = mask_select_emb(pretrain_embedding_2, valmask, device)
                testemb_2 = mask_select_emb(pretrain_embedding_2, testmask, device)
                
                trainemb_3 = mask_select_emb(pretrain_embedding_3, trainmask, device)
                valemb_3 = mask_select_emb(pretrain_embedding_3, valmask, device)
                testemb_3 = mask_select_emb(pretrain_embedding_3, testmask, device)
                
                trainemb_4 = mask_select_emb(pretrain_embedding_4, trainmask, device)
                valemb_4 = mask_select_emb(pretrain_embedding_4, valmask, device)
                testemb_4 = mask_select_emb(pretrain_embedding_4, testmask, device)
                ...
                #else:
                para = []
                for name,p in model_0.named_parameters():
                    if "prompt" in name:
                        # print("xxxxxxxx")
                        p.requires_grad = True
                        para.append(p)
               
                optimizer = torch.optim.AdamW(para, lr=train_config["lr"],
                                            weight_decay=train_config["weight_decay"], amsgrad=True)
                optimizer_2 = torch.optim.AdamW(weight_.parameters(), lr=train_config["weight_lr"],
                                            weight_decay=train_config["weight_decay"], amsgrad=True)
                optimizer.zero_grad()
                optimizer_2.zero_grad()
                scheduler = None
                ...
                for epoch in range(train_config["epochs"]):
                    mean_reg_loss, mean_bp_loss, _time, accfold,c_embedding = train(weight_, optimizer, optimizer_2,scheduler, "train", device,
                                                            train_config, epoch, train_config["train_label_num"], trainemb_1,trainemb_2,trainemb_3,trainemb_4,trainlabel, logger=logger, writer=writer,man_weight= man_weight)
                    total_train_time += _time                                                                                
                    torch.save(model_0.state_dict(), os.path.join(save_model_dir, '00_epoch%d.pt' % (epoch)))
                   
                    torch.save(weight_.state_dict(), os.path.join(save_model_dir, 'weight_epoch%d.pt' % (epoch)))
                    ...
                    mean_reg_loss, mean_bp_loss, evaluate_results, _time,accfold, macroFfold, weightedFfold,c_embedding = \
                        evaluate(weight_, "val", device, train_config, epoch, c_embedding,
                                train_config["val_label_num"], valemb_1,valemb_2, valemb_3,valemb_4,vallabel, logger=logger, writer=None,man_weight= man_weight)
                ...
                model_0.load_state_dict(torch.load(os.path.join(save_model_dir, '00_epoch%d.pt' % (best_epoch))))
               
                weight_.load_state_dict(torch.load(os.path.join(save_model_dir, 'weight_epoch%d.pt' % (best_epoch))))
                ...
                mean_reg_loss, mean_bp_loss, evaluate_results, _time, acctest, macroFtest, weightedFtest,c_embedding = \
                    evaluate(weight_, "test", device, train_config, epoch, best_c_embedding,
                                    train_config["val_label_num"], testemb_1,testemb_2, testemb_3,testemb_4,testlabel, debug=True, logger=logger, writer=None,man_weight= man_weight)
            ...
            # np.savetxt('%s_%s_%s_new%s_times%d.txt'%(record[mix_num],dataset_type,mix_,record[adj_self_weight[weight]],times),total_rec,fmt='%s')
            np.savetxt('C_mix_GCL_flickr_times%d.txt'%(times),total_rec,fmt='%s')
