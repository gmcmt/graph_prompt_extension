...
class node_prompt_layer_linear_sum(nn.Module):
    ...
    def forward(self, graph_embedding, graph_len):
        graph_embedding=self.linear(graph_embedding)
        #graph_prompt_result=torch.nn.functional.normalize(graph_embedding,dim=1)
        return graph_embedding
...
#7.26 error verison 算法错误，没法解释，但可能其结果是有用的
'''class graph_prompt_layer_weighted(nn.Module):
    def __init__(self,max_n_num):
        super(graph_prompt_layer_weighted, self).__init__()
        #assign a weight for each node while aggregating the nodes' embedding to get graph embedding
        #max_n_num+1从而使得graph embedding和weight对其情况下forward中align可以生效
    ...
    def forward(self, graph_embedding, graph_len):
        ...
        temp1 = weight * temp1
        graph_embedding=torch.matmul(graph_embedding,temp1)
        #prompt: mean
        graph_prompt_result=graph_embedding.mean(dim=1)
        return graph_prompt_result'''
