    ...
    for weight in range(len(adj_self_weight)):
        ...
        for times in range(5):
            ...
            if train_config["pretrain_model"] == "GIN":
                ...
                pre_train_model_d.load_state_dict(torch.load(os.path.join(save_pretrain_model_dir, "%s_flickr_DGI_%s.pt"%(train_config["d_p"],times))))
                torch.save(pre_train_model_d.gin.state_dict(), os.path.join(save_pretrain_model_dir, "gin_DGI_%s.pt"%(times)))
            ...
            for count in range(train_config["few_shot_tasknum"]):
                ...
                model_0.load_state_dict(torch.load(os.path.join(save_pretrain_model_dir, 'gin_DGI_%s.pt'%(times)),map_location='cuda:0'),strict = False)
            ...
            np.savetxt('C_mix_DGI_flickr_times%d.txt'%(times),total_rec,fmt='%s')
